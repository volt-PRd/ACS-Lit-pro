...
package com.ACSlit.pro.desugaring

import com.android.build.api.instrumentation.ClassContext
import com.ACSlit.pro.desugaring.dsl.MethodOpcode
import com.ACSlit.pro.desugaring.dsl.ReplaceMethodInsn
import com.ACSlit.pro.desugaring.dsl.ReplaceMethodInsnKey
import org.objectweb.asm.MethodVisitor
import org.slf4j.LoggerFactory

class DesugarMethodVisitor
@JvmOverloads
constructor(
  ...
  override fun visitMethodInsn(
    ...
    val replacement = findReplacementForClsOrSupercls(opcode, owner, name, descriptor)
    if (replacement == null) {
    ...
    val toClass = replacement.toClass.requireValue("toClass", methodDesc).replace('.', '/')

    val toMethod = replacement.toMethod.requireValue("toMethod", methodDesc)
    val toDescriptor = replacement.toMethodDescriptor.requireValue("toMethodDescriptor", methodDesc)
    ...
    super.visitMethodInsn(toOpcode.opcode, toClass, toMethod, toDescriptor, false)
  }

  private fun findReplacementForClsOrSupercls(
      opcode: Int,
      owner: String?,
      method: String?,
      descriptor: String?,
    ...
    val classData = classContext.loadClassData(owner.replace('/', '.')) ?: return null

    // case 2: check if we have a replacement for a superclass
    for (superclass in classData.superClasses) {
      val replacement = findReplacementForClsOrSupercls(opcode, superclass, method, descriptor)
      if (replacement != null) {
        return replacement
      }
    }
    ...
    for (iface in classData.interfaces) {
      val replacement = findReplacementForClsOrSupercls(opcode, iface, method, descriptor)
      if (replacement != null) {
        return replacement
      }
    }
  ...
  private fun findReplacement(
    ...
    return replaceMethodInsn.takeIf {
      ...
      (methodOpcode == MethodOpcode.ANY || methodOpcode.opcode == opcode) &&
          methodDescriptor == descriptor
    }
  }

  private fun <T : Any?> T.requireValue(propName: String, methodDesc: String): T {
    return this?.let {
