...
import com.ACSlit.pro.javac.services.fs.AndroidFsProviderImpl;
...
public class SourceFileManager extends ForwardingJavaFileManager<JavacFileManager> {
  ...
  private SourceFileManager(final ModuleProject module) {
    ...
    com.ACSlit.pro.projectdata.logs.LogStream.INSTANCE.emitLineBlocking("Configuring source paths...");
    setLocationLogError(StandardLocation.SOURCE_PATH, module.getCompileSourceDirectories());
    
    com.ACSlit.pro.projectdata.logs.LogStream.INSTANCE.emitLineBlocking("Configuring classpaths...");
    setLocationLogError(StandardLocation.CLASS_PATH, configureClasspaths(module));
    
    com.ACSlit.pro.projectdata.logs.LogStream.INSTANCE.emitLineBlocking("Indexing project files...");
    listLocations(EnumSet.of(StandardLocation.CLASS_PATH, StandardLocation.PLATFORM_CLASS_PATH));
    
    com.ACSlit.pro.projectdata.logs.LogStream.INSTANCE.emitLineBlocking("File manager initialization complete!");
    
    // Clear indexing flag after initialization
    com.ACSlit.pro.projectdata.state.lsp.Index.INSTANCE.setIsIndexing(false);
  }
  ...
  private static SourceFileManager createForModule(@NonNull ModuleProject project) {
    ...
    com.ACSlit.pro.projectdata.state.lsp.Index.INSTANCE.setIsIndexing(true);
    
    com.ACSlit.pro.projectdata.logs.LogStream.INSTANCE.emitLineBlocking("Initializing file manager for: " + project.getName());
