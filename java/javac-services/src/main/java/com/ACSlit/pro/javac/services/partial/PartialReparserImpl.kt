...
package com.ACSlit.pro.javac.services.partial

import com.ACSlit.pro.javac.services.compiler.JavacFlowListener
import com.ACSlit.pro.javac.services.util.ReparserUtils
import com.ACSlit.pro.javac.services.visitors.FindAnonymousVisitor
import com.ACSlit.pro.javac.services.visitors.TranslateMethodPositionsVisitor
...
class PartialReparserImpl : PartialReparser {
  ...
  override fun reparseMethod(
    ...
    try {
      val l = com.ACSlit.pro.javac.services.NBLog.instance(context)
  ...
  private fun newParser(
    ...
    val factory =
        com.ACSlit.pro.javac.services.NBParserFactory.instance(context)
            as com.ACSlit.pro.javac.services.NBParserFactory
    val scannerFactory = ScannerFactory.instance(context)
    val cancelService = com.ACSlit.pro.javac.services.CancelService.instance(context)
    val lexer = scannerFactory.newScanner(buf, true)
    if (
        endPositions is com.ACSlit.pro.javac.services.NBParserFactory.NBJavacParser.EndPosTableImpl
    ) {
      endPositions.resetErrorEndPos()
    }

    return object :
        com.ACSlit.pro.javac.services.NBParserFactory.NBJavacParser(
            factory,
            lexer,
            true,
            false,
