 ...
 *   string resources.
 * @param partialRFile file to output symbols defined in the resource file (currently only supported
 *   for non-raw XML files (e.g. values or layouts) to match AAPT2's behaviour)
...
fun canCompileResourceInJvm(file: File, requirePngCrunching: Boolean): Boolean {
  ...
  if (
      pathData.resourceDirectory == VALUES_DIRECTORY_PREFIX && pathData.extension == XML_EXTENSION
  ) {
...
private fun getCompileMethod(
  ...
  if (
    ...
    if (type == null) {
      val errorMsg =
          "Invalid resource type '${pathData.resourceDirectory}'" +
 ...
 *
 * After all resources are extracted from the given xml, if pseudolocalization is set, pseudolocale
 * strings are created for each default string resource in the xml. The result is flattened to a
  ...
  pathData.file.inputStream().use {
    try {
      ...
      throw ResourceCompilationException(
          "Failed to compile values resource file ${pathData.file}",
          e,
      )
    }
        ...
        pkg.groups.forEach { group ->
          ...
          if (group.type != AaptResourceType.MACRO) {
            ...
            group.entries.forEach { entry ->
              ...
              builder.appendLine(
                  "${visibility.getName()} $javaType ${group.type.tagName} ${entry.key}"
              )
...
private fun getVisibility(
    values: Collection<ResourceEntry>,
 ...
 *
 * Finally, the processed file is written to the output directory. It is possible that no processing
 * is done to the png file, and if so it will just be written exactly as if it were a raw file.
 *
