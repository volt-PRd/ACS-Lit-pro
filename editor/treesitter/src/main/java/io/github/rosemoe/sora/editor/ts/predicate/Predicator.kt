...
class Predicator(private val query: TSQuery) {
  ...
  init {
    for (i in 0 until query.patternCount) {
      patternPredicates.add(
          query.getPredicatesForPattern(i).map {
            when (it.type) {
              TSQueryPredicateStep.Type.String ->
                  TsClientPredicateStep(it.type, query.getStringValueForId(it.valueId))

              TSQueryPredicateStep.Type.Capture ->
                  TsClientPredicateStep(it.type, query.getCaptureNameForId(it.valueId))
  ...
  fun doPredicate(
      ...
      text: CharSequence,
      match: TSQueryMatch,
      syntheticCaptureContainer: TsSyntheticCaptureContainer =
          TsSyntheticCaptureContainer.EMPTY_IMMUTABLE_CONTAINER,
  ): Boolean {
    ...
    for (i in description.indices) {
      if (description[i].predicateType == TSQueryPredicateStep.Type.Done) {
        ...
        val subPredicateStep =
            if (tail == 0 && i + 1 == description.size) description
            else description.subList(tail, i + 1)
        for (j in predicates.indices) {
          val predicate = predicates[j]
          when (
              predicate.doPredicate(query, text, match, subPredicateStep, syntheticCaptureContainer)
          ) {
