...
/**
 * Theme for tree-sitter. This is different from
 * [io.github.rosemoe.sora.widget.schemes.EditorColorScheme]. It is only used for colorizing spans
 * in tree-sitter module. The real colors are still stored in editor color schemes. As what
 * tree-sitter do, we try to match the longest scope. For example, if 'variable' and
 * 'variable.builtin' rule are both defined, Query 'variable.builtin' and 'variable.builtin.this'
 * will get 'variable.builtin' rule. The theme also provide a fallback. You may call
 * [TsTheme.putStyleRule] with a rule of length 0 to set fallback color scheme. Note that colors of
 * 'locals.definition', 'locals.reference', etc. can not be set by this theme object.
 *
 * @author Rosemoe
 */
