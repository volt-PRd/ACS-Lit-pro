 ...
 *
 * Note that you must use ASCII characters in your scm sources. Otherwise, an
 * [IllegalArgumentException] is thrown. Be careful that this should be closed to avoid native
 * memory leaks.
 *
 ...
 * @param codeBlocksScmSource The scm source for capturing code blocks. All captured nodes are
 *   considered to be a code block. Capture named with '.marked' suffix will have its last terminal
 *   node's start position as its scope end
...
open class TsLanguageSpec(
  ...
  val blocksQuery =
      if (codeBlocksScmSource.isBlank()) {
        TSQuery.EMPTY
      } else TSQuery.create(language, codeBlocksScmSource)

  val bracketsQuery =
      if (bracketsScmSource.isBlank()) {
        TSQuery.EMPTY
