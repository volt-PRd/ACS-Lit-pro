 ...
 */
package com.ACSlit.pro.actions.internal
...
import com.ACSlit.pro.actions.OnActionClickListener
import com.ACSlit.pro.actions.locations.CodeActionsMenu
...
class DefaultActionsRegistry : ActionsRegistry() {
  ...
  private val actionsCoroutineScope =
      CoroutineScope(Dispatchers.Default) + CoroutineName("DefaultActionsRegistry")
  ...
  private fun addActionToMenu(
    ...
    if (action !is ActionMenu) {
      item.setOnMenuItemClickListener { onClickListener.onClick(this, action, it, data) }
    }
  }
  ...
  fun executeAction(action: ActionItem, data: ActionData): Job {
    ...
    return actionsCoroutineScope
        .launch(context) {
          val result = withStopWatch("Action '${action.id}'") { action.execAction(data) }
