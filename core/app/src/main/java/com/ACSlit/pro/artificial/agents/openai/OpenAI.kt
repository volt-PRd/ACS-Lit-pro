...
package com.ACSlit.pro.artificial.agents.openai

import android.content.Context
import com.ACSlit.pro.artificial.services.ArtificialService
import com.ACSlit.pro.artificial.rules.WritingRules
import com.ACSlit.pro.artificial.project.awareness.ProjectTreeResult
import com.ACSlit.pro.artificial.file.AIFileWriter
import com.ACSlit.pro.artificial.file.FileWriteResult
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.withContext
import java.io.File
import com.ACSlit.pro.artificial.agents.Agents
import org.json.JSONObject
import org.json.JSONArray
import java.net.HttpURLConnection
import java.net.URL
import com.ACSlit.pro.artificial.exceptions.*
import com.ACSlit.pro.artificial.agents.AIAgent
import com.ACSlit.pro.artificial.agents.AIAgentRegistry
import com.ACSlit.pro.artificial.secrets.ApiKey
import com.ACSlit.pro.artificial.agents.ModificationAttempt
...
class OpenAI : AIAgent {
        ...
        try {
          ...
          when {
            responseCode == 429 || errorType.contains("rate_limit") || errorCode.contains("rate_limit") -> 
              throw com.ACSlit.pro.artificial.exceptions.RateLimitException("OpenAI rate limit exceeded: $errorMessage")
            errorType.contains("insufficient_quota") || errorMessage.contains("quota") || errorMessage.contains("billing") -> 
              throw com.ACSlit.pro.artificial.exceptions.QuotaExceededException("OpenAI quota exceeded: $errorMessage")
            errorType.contains("invalid_api_key") || errorCode.contains("invalid_api_key") -> 
              throw com.ACSlit.pro.artificial.exceptions.InvalidApiKeyException("Invalid OpenAI API key: $errorMessage")
            responseCode == 401 -> 
              throw com.ACSlit.pro.artificial.exceptions.InvalidApiKeyException("OpenAI authentication failed: $errorMessage")
            else -> 
              throw Exception("OpenAI API error ($responseCode) - Type: $errorType, Code: $errorCode, Message: $errorMessage")
          }
        } catch (e: com.ACSlit.pro.artificial.exceptions.RateLimitException) {
          throw e
        } catch (e: com.ACSlit.pro.artificial.exceptions.QuotaExceededException) {
          throw e
        } catch (e: com.ACSlit.pro.artificial.exceptions.InvalidApiKeyException) {
          throw e
        } catch (e: Exception) {
  ...
  fun parseAndApplyModifications(response: String, capturedStates: Map<String, String>): List<FileModification> {
    ...
    val parser = com.ACSlit.pro.artificial.parser.SnippetParser()
