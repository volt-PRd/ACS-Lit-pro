...
object NavigationBar {
   ...
   * [NavigationBarInteractionMode]. Depending on the Android version and OEM implementation, users
   * might change the interaction mode via system settings: System>Gestures>System Navigation. This
   * can lead to conflicts with apps that use specific system-gestures internally for navigation (i.
   * e. swiping), especially if the Android 10 full screen gesture mode is enabled.
   *
  ...
  fun getInteractionMode(context: Context): Int {
    ...
    return if (resourceId > 0) resources.getInteger(resourceId) else MODE_THREE_BUTTONS
  }
