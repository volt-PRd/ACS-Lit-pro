...
package com.ACSlit.pro.logsender;
...
import com.ACSlit.pro.logsender.utils.LogReader;
import com.ACSlit.pro.logsender.utils.Logger;
...
public final class LogSender extends ILogSender.Stub implements ServiceConnection {
    ...
    public static final String SERVICE_ACTION = "com.ACSlit.pro.LOG_SERVICE_ACTION";
    ...
    public static final String PACKAGE_ANDROIDIDE = "com.ACSlit.pro";
    ...
    public void onServiceDisconnected(ComponentName name) {
        Logger.warn("Service disconnected unexpectedly");
        tryDisconnect(false); 
    }
     ...
     * @return Whether the binding was successful or not. This returns <code>true</code> if {@link
     *     Context#bindService(Intent, ServiceConnection, int)} returns true or if the log sender is
     *     already bound to the service. Otherwise <code>false</code> is returned.
     */
    ...
    boolean tryReconnect(Context context) {
        if (isConnected()) {
        ...
        if (isBinding()) {
            Logger.debug("Already binding, waiting...");
            return true;
        }

        Logger.info("Attempting to reconnect to log receiver...");
        return bind(context);
    }
