...
import com.ACSlit.pro.app.BaseIDEActivity;
...
/**
 * An activity to show reports in markdown format as per CommonMark spec based on config passed as
 * {@link ReportInfo}. Add Following to `AndroidManifest.xml` to use in an app:
 * {@code `<activity android:name="com.termux.shared.activities.ReportActivity"
 * android:theme="@style/Theme.AppCompat.TermuxReportActivity"
 * android:documentLaunchMode="intoExisting" />` } and
 * {@code `<receiver
 * android:name="com.termux.shared.activities.ReportActivity$ReportActivityBroadcastReceiver"
 * android:exported="false" />` } Receiver **must not** be `exported="true"`!!!
 * <p>
 * Also make an incremental call to
...
public class ReportActivity extends BaseIDEActivity {
  ...
  private static final String EXTRA_REPORT_INFO_OBJECT_FILE_PATH =
      CLASS_NAME + ".EXTRA_REPORT_INFO_OBJECT_FILE_PATH";
  ...
  private void updateUI() {
    ...
    if (mBundle.containsKey(EXTRA_REPORT_INFO_OBJECT_FILE_PATH)) {
      ...
      Logger.logVerbose(LOG_TAG,
          ReportInfo.class.getSimpleName() + " serialized object will be read from file at path \""
              + mReportInfoFilePath + "\"");
      if (mReportInfoFilePath != null) {
        try {
          ...
          Logger.logStackTraceWithMessage(LOG_TAG,
              "Failure while getting " + ReportInfo.class.getSimpleName()
  ...
  private void generateReportActivityMarkdownString() {
    ...
    if (reportMarkdownStringSize > ACTIVITY_TEXT_SIZE_LIMIT_IN_BYTES) {
      Logger.logVerbose(LOG_TAG,
          mReportInfo.reportTitle + " report string size " + reportMarkdownStringSize
  ...
  public static class NewInstanceResult {
    ...
    /**
     * An intent that can should be adding as the {@link android.app.Notification#deleteIntent} by a
     * call to {@link android.app.PendingIntent#getBroadcast(Context, int, Intent, int)} so that
     * {@link ReportActivityBroadcastReceiver} can do cleanup of
     * {@link #EXTRA_REPORT_INFO_OBJECT_FILE_PATH}.
     */
   ...
   */
  public static void startReportActivity(@NonNull final Context context,
  ...
  /**
   * Get content and delete intents for the {@link ReportActivity} that can be used to start it and
   * do cleanup.
   * <p>
   * If {@link ReportInfo} size is too large, then a TransactionTooLargeException will be thrown so
   * its object may be saved to a file in the {@link Context#getCacheDir()}. Then when activity
   * starts, its read back and the file is deleted in {@link #onDestroy()}. Note that files may
   * still be left if {@link #onDestroy()} is not called or doesn't finish. A separate cleanup
   * routine is implemented from that case by
   ...
   */
  @NonNull
  public static NewInstanceResult newInstance(@NonNull final Context context,
      @NonNull final ReportInfo reportInfo) {
    ...
    if (size > DataUtils.TRANSACTION_SIZE_LIMIT_IN_BYTES) {
      ...
      String reportInfoFilePath =
          reportInfoDirectoryPath + "/" + CACHE_FILE_BASENAME_PREFIX + reportInfo.reportTimestamp;
      Logger.logVerbose(LOG_TAG, reportInfo.reportTitle + " " + ReportInfo.class.getSimpleName()
          + " serialized object size " + size + " is greater than "
          + DataUtils.TRANSACTION_SIZE_LIMIT_IN_BYTES + " and it will be written to file at path \""
          + reportInfoFilePath + "\"");
      Error error = FileUtils.writeSerializableObjectToFile(ReportInfo.class.getSimpleName(),
          reportInfoFilePath, reportInfo);
      if (error != null) {
  ...
  private static Intent createContentIntent(@NonNull final Context context,
      final ReportInfo reportInfo, final String reportInfoFilePath) {
  ...
  private static Intent createDeleteIntent(@NonNull final Context context,
  ...
  private static String getReportInfoDirectoryPath(Context context) {
    ...
    return FileUtils.getCanonicalPath(context.getCacheDir().getAbsolutePath(), null) + "/"
        + CACHE_DIR_BASENAME;
  }
  ...
  private static void deleteReportInfoFile(Context context, String reportInfoFilePath) {
    ...
    if (!reportInfoFilePath.equals(reportInfoDirectoryPath) && reportInfoFilePath.startsWith(
        reportInfoDirectoryPath + "/")) {
      Logger.logVerbose(LOG_TAG,
          "Deleting " + ReportInfo.class.getSimpleName() + " serialized object file at path \""
              + reportInfoFilePath + "\"");
      Error error = FileUtils.deleteRegularFile(ReportInfo.class.getSimpleName(),
          reportInfoFilePath, true);
      if (error != null) {
        Logger.logErrorExtended(LOG_TAG, error.toString());
      }
    } else {
   ...
   * notification has still not been opened after x days that's using a PendingIntent to
   * ReportActivity, then opening the notification will throw a file not found error, so choose days
   * value appropriately or check if a notification is still active if tracking notification ids.
   * The {@link Context} object passed must be of the same package with which
   * {@link #newInstance(Context, ReportInfo)} was called since a call to
   * {@link Context#getCacheDir()} is made.
   *
   * @param context       The {@link Context} for operations.
   * @param days          The x amount of days before which files should be deleted. This must be
   *                      `>=0`.
   * @param isSynchronous If set to {@code true}, then the command will be executed in the caller
   *                      thread and results returned synchronously. If set to {@code false}, then a
   *                      new thread is started run the commands asynchronously in the background
   *                      and control is returned to the caller thread.
  ...
  private static Error deleteReportInfoFilesOlderThanXDaysInner(@NonNull final Context context,
      int days) {
    ...
    Logger.logVerbose(LOG_TAG, "Deleting " + ReportInfo.class.getSimpleName()
        + " serialized object files under directory path \"" + reportInfoDirectoryPath
        + "\" older than " + days + " days");
    return FileUtils.deleteFilesOlderThanXDays(ReportInfo.class.getSimpleName(),
        reportInfoDirectoryPath, null, days, true, FileType.REGULAR.getValue());
  }


  /**
   * The {@link BroadcastReceiver} for {@link ReportActivity} that currently does cleanup when
   * {@link android.app.Notification#deleteIntent} is called. It must be registered in
   * `AndroidManifest.xml`.
   */
