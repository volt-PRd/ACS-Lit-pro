...
/**
 * Attribute adapter for [TextInputLayout].
 *
 * @author Akash Yadav
 */
@ViewAdapter(TextInputLayout::class)
@IncludeInDesigner(group = LAYOUTS)
open class EditTextLayoutAdapter<T : TextInputLayout> : LinearLayoutAdapter<T>() {
  override fun createUiWidgets(): List<UiWidget> {
    return listOf(
        UiWidget(TextInputLayout::class.java, string.widget_edittext, drawable.ic_widget_edittext)
    )
  }
}
